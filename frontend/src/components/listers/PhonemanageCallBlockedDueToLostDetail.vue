<template>
    <v-card outlined>
        <v-card-title>
            CallBlockedDueToLost # {{item._links.self.href.split("/")[item._links.self.href.split("/").length - 1]}}
        </v-card-title>
...
<script>
    ...
    export default {
        name: 'PhonemanageCallBlockedDueToLostDetail',
        components:{},
        props: {
        },
        data: () => ({
        ...
        async created() {
            ...
            var temp = await axios.get(axios.fixUrl('/callBlockedDueToLosts/' + params.id))
            if(temp.data) {
                me.item = temp.data
            }
        },

<template>
    <v-card outlined>
        ...
        <v-card-title v-if="value._links">
            CallBlockedDueToLost # {{decode(value._links.self.href.split("/")[value._links.self.href.split("/").length - 1])}}
        </v-card-title >
        <v-card-title v-else>
...
<script>
    ...
    export default {
        name: 'PhonemanageCallBlockedDueToLost',
        components:{
        },
        props: {
        ...
        methods: {
            ...
            async save(){
                try {
                    ...
                    if(!this.offline) {
                        if(this.isNew) {
                            temp = await axios.post(axios.fixUrl('/callBlockedDueToLosts'), this.value)
                        } else {
